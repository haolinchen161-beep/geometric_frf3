...
class GeometricHDF5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.freq_sampling:
            # 分层采样: 峰值区域密集 + 全频段均匀覆盖
            # 两峰真实范围: 62~68Hz, 393~425Hz (E/ρ变化±4%)
            # 放宽到±10Hz覆盖完整斜坡(上升+下降段)
            f_raw = (freq + 1) / 2 * (500 - 1) + 1  # 逆归一化: [-1,1] -> [1,500] Hz
            peak1_mask = (f_raw >= 55) & (f_raw <= 80)
            peak2_mask = (f_raw >= 390) & (f_raw <= 430)
            peak_indices = torch.where(peak1_mask | peak2_mask)[0]
            other_indices = torch.where(~(peak1_mask | peak2_mask))[0]

            n_peak = min(20, len(peak_indices))
            n_uniform = self.freq_sampling_limit - n_peak

            peak_sample = peak_indices[torch.randint(0, len(peak_indices), (n_peak,))]
            uniform_sample = other_indices[torch.linspace(0, len(other_indices)-1, n_uniform, dtype=torch.long)]
            freq_idx = torch.cat([peak_sample, uniform_sample])
            freq = freq[freq_idx]
